...
Use `Europe` or `Asia` for the corresponding LG regional API endpoint. The tool calls `GET /devices` first and saves **every device and its reported type** in `00_device_type_inventory.json`. This is the list observed on your LG account, not a global catalog of every type LG supports. The console prints every device number, type, and model. The tool then calls `GET /push`, `GET /push/devices`, `GET /event`, and profile, state, and energy-profile GETs for priority devices. Use `-IncludeEnergyUsage -EnergyProperty PROPERTY_NAME` to try today's daily energy GET after identifying a property in the energy profile. API errors are recorded with HTTP status and LG's redacted JSON error body.
