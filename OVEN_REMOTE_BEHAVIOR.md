...
When `remoteControlEnabled` is false, the oven command selects expose only their display state and reject direct control service calls. The oven target-temperature number becomes unavailable and its service call is blocked. Before any oven command, the integration fetches fresh state and checks remote readiness again. The owner should test that the flag becomes false after the oven is turned off and true after the dial is reset. No control POST was sent during implementation.
