...
| Washer/dryer combo | Observe washing and drying state without sending START for a dry-only load. | START now checks a fresh remote-enable state, but LG's profile has no writable dry-cycle selection. The owner observed START begin washing despite Dry being selected on the dial. | Current state through wash/dry transition, completion event, actual machine action. |
